...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if max_completion_length is not None:
            warnings.warn(
                "You passed `max_completion_length` to the DPOTrainer, the value you passed will override the one in the `DPOConfig`."
            )
            args.max_completion_length = max_completion_length
        if args.max_completion_length is None:
            warnings.warn(
                "`max_completion_length` is not set in the DPOConfig's init"
                " it will default to `1000000` by default, but you should do it yourself in the future.",
                UserWarning,
            )
            args.max_completion_length = 1000000
    ...
    def tokenize_row(self, feature, model: Optional[Union[PreTrainedModel, nn.Module]] = None) -> Dict:
        ...
        if not self.is_encoder_decoder:
            ...
            # truncate the response with max_completion_length
            for answer_tokens in [chosen_tokens, rejected_tokens]:
                if len(answer_tokens["input_ids"]) > self.max_completion_length:
                    for k in ["input_ids", "attention_mask"]:
                        answer_tokens[k] = answer_tokens[k][: self.max_completion_length]
    ...
    def get_batch_logps(
        ...
        if use_poet:
            batch_size = int(labels.shape[0] / 2)
            chosen_loss_mask = loss_mask[: batch_size].clone()
            rejected_loss_mask = loss_mask[batch_size :].clone()
            share_loss_mask = chosen_loss_mask & rejected_loss_mask
            use_poet_loss_mask = torch.cat((share_loss_mask, share_loss_mask), dim=0)
        else:
            use_poet_loss_mask = torch.ones_like(loss_mask, dtype=loss_mask.dtype, device=labels.device)

        # dummy token; we'll ignore the losses on these tokens later
        labels[labels == label_pad_token_id] = 0

        per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
        loss_mask = loss_mask * use_poet_loss_mask
